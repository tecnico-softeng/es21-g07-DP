...
Cypress.Commands.add('cleanItemConnectionQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='item_connection' where title like '%${questionName}%')
                , consToDelete AS (SELECT id FROM  connection_links WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id))
                , connections AS (DELETE FROM connection_links where id IN (SELECT id FROM consToDelete)) 
                , connections1 AS (DELETE FROM question_details_connections where connections_id IN (SELECT id FROM consToDelete)) 
                , itemsToDelete AS (SELECT id FROM  connection_items WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id))
                , items AS (DELETE FROM connection_items where id IN (SELECT id FROM itemsToDelete)) 
                , groupLeft AS (DELETE FROM question_details_group_left where group_left_id IN (SELECT id FROM itemsToDelete)) 
                , groupRight AS (DELETE FROM question_details_group_right where group_right_id IN (SELECT id FROM itemsToDelete)) 
                , det AS (DELETE FROM question_details WHERE question_id in (SELECT question_id FROM toDelete))
                DELETE FROM questions WHERE id IN (SELECT question_id FROM toDelete);`);
});

Cypress.Commands.add('cleanOpenQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='open' where title like '%${questionName}%')
                , det AS (DELETE FROM question_details WHERE question_id in (SELECT question_id FROM toDelete))
              DELETE FROM questions WHERE id IN (SELECT question_id FROM toDelete);`);
});
