...
public abstract class LatexVisitor implements Visitor {
    ...
    public void visitQuestionDetails(OpenQuestion question) {
        this.result += String.format("\n\tTeacher Answer: %s", question.getTeacherAnswer()) + "\n\n";
        this.result += "\\end{ClosedQuestion}\n}\n\n";
    }

    @Override
    public void visitQuestionDetails(ItemConnectionQuestion question) {
        
        this.result += "\tLeft Group\n";
    ...
    public void visitItem(ConnectionItem item) {
        this.result += String.format("\t\\item:{%s}\n", item.getContent());
    }

    @Override
    public void visitConnectionLink(ConnectionLink link){
        this.result += String.format("\t\\link:{leftGroup option no.%d - rightGroup option no.%d}\n", link.getOrigin()+1, link.getDestiny()+1);
    }
