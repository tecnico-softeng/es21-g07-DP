...
public class QuestionsXmlImport {
    ...
    private void importQuestion(Element questionElement, Course course) {
        ...
        switch (type) {
            ...
            case Question.QuestionTypes.CODE_ORDER_QUESTION:
                ...
                break;
            case Question.QuestionTypes.ITEM_CONNECTION_QUESTION:
                questionDetailsDto = importItemConnectionQuestion(questionElement);
                break;
            case Question.QuestionTypes.OPEN_QUESTION:
                questionDetailsDto = importOpenQuestion(questionElement);
                break;
            default:
    ...
    private QuestionDetailsDto importItemConnectionQuestion(Element questionElement) {
        ItemConnectionQuestionDto questionDto = new ItemConnectionQuestionDto();
        List<ConnectionItemDto> leftGroup = new ArrayList<>();
        List<ConnectionItemDto> rightGroup = new ArrayList<>();
        List<ConnectionLinkDto> connections = new ArrayList<>();

        for (Element itemElement : questionElement.getChild("leftItems").getChildren("item")) {
            ConnectionItemDto itemDto = new ConnectionItemDto();
            String itemContent = itemElement.getAttributeValue("content");

            itemDto.setContent(itemContent);
            leftGroup.add(itemDto);
        }
        for (Element itemElement : questionElement.getChild("rightItems").getChildren("item")) {
            ConnectionItemDto itemDto = new ConnectionItemDto();
            String itemContent = itemElement.getAttributeValue("content");
        ...
        questionDto.setOptions(leftGroup, rightGroup);
        for (Element itemElement : questionElement.getChild("connectionLinks").getChildren("link")) {
            Integer linkOrigin = Integer.valueOf(itemElement.getAttributeValue("origin"));
            Integer linkDestiny = Integer.valueOf(itemElement.getAttributeValue("destiny"));
            ConnectionLinkDto linkDto = new ConnectionLinkDto(linkOrigin, linkDestiny);
            connections.add(linkDto);
        }
        questionDto.setConnections(connections);
        return questionDto;
    }

    private QuestionDetailsDto importOpenQuestion(Element questionElement) {
        OpenQuestionDto questionDto = new OpenQuestionDto();
        questionDto.setTeacherAnswer(questionElement.getChildText("teacherAnswer"));
        return questionDto;
    }
